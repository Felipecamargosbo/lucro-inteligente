  ...
  head: () => ({
    meta: [
      { title: "Fulfillment e cobertura no Full | NEXO Rentabilidade" },
      {
        name: "description",
        content:
          "Controle o capital investido no Full, cobertura em dias, alertas de ruptura e estoque parado nos galpões dos marketplaces.",
      },
      {
        property: "og:title",
      ...
      },
      {
        ...
        content:
          "Inteligência de fulfillment: unidades alocadas nos galpões, ruptura iminente e produtos sem giro.",
      },
...
/** Os 4 status oficiais do fulfillment, em ordem de prioridade. */
function statusFull(item: ItemEstoqueDetalhado): StatusOficial {
  if (item.quantidade === 0)
    return { texto: "Esgotado", cor: "bg-loss-soft text-loss", acao: "Comprar Estoque" };
  if (item.quantidade < 10 || item.coberturaDias < 10)
    return { texto: "Atenção", cor: "bg-warning-soft text-foreground", acao: "Estoque Baixo" };
  if (item.coberturaDias > 60)
    return { texto: "Sem Giro", cor: "bg-warning text-foreground", acao: "Suficiente" };
  return { texto: "Estoque Saudável", cor: "bg-profit-soft text-profit", acao: "Suficiente" };
}

function Fulfillment() {
  const itens = fulfillmentService.listarDetalhado();
  const resumo = fulfillmentService.resumo();
  ...
  const linhasExport = filtrados.map((i) => ({
    SKU: i.sku,
    Produto: i.produto,
    "Qtd no Full": i.quantidade,
    "Vendas/dia": i.vendasDia,
    "Cobertura (dias)": i.coberturaDias,
    "Valor no Full": i.valorEstoque,
    Status: statusFull(i).texto,
  }));
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <CardKpi
          titulo="Capital investido no Full"
          valor={formatBRL(resumo.capitalInvestido)}
          detalhe="CMV das unidades alocadas nos galpões dos marketplaces"
          dica="É o dinheiro imobilizado em produtos que já estão no centro de distribuição."
        />
        <CardKpi
          titulo="SKUs em Full"
          valor={`${formatNumero(resumo.skusFull)} itens`}
          detalhe="Produtos com estoque alocado em fulfillment"
        />
        <CardKpi
          titulo="SKUs em alerta de ruptura"
          valor={`${resumo.skusRuptura} itens`}
          detalhe="Produtos no Full com menos de 10 unidades"
          dica="Estoque baixo no galpão: risco de perder o selo Full e vendas."
        />
        <CardKpi
          titulo="Estoque parado / sem giro"
          valor={`${formatNumero(resumo.unidadesParadas)} un`}
          detalhe="Unidades com mais de 60 dias de cobertura no galpão"
        />
      </div>

      <Painel
        titulo="Produtos e cobertura no fulfillment"
        descricao="Quantos dias de venda ainda cabem no estoque alocado nos galpões"
        acoes={<ExportarDados nomeArquivo="fulfillment" linhas={linhasExport} />}
      >
          ...
          <table className="w-full min-w-[1000px] text-left">
            <thead>
              <tr className="border-b bg-muted/50 text-[10px] uppercase tracking-wide text-muted-foreground">
                ...
                <th className="px-4 py-3 text-right font-bold">Qtd no Full</th>
                ...
                <th className="px-4 py-3 text-right font-bold">Valor no Full</th>
